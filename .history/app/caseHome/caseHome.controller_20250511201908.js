...
export const createNewCaseHome = asyncHandler(async (req, res) => {
  const {
    name, price, img, developerIds, categoryIds, date, website,
  } = req.body;
  ...
  try {
    const images = (img || []).map((image) => (typeof image === 'object' ? `/uploads/${image.rawFile.path}` : image));
...
export const updateCaseHome = asyncHandler(async (req, res) => {
  const {
    name, price, img, categoryIds, developerIds, date, website,
  } = req.body;
